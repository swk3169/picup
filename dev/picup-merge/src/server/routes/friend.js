/* 
 GET /friend => 내 친구 목록(내가 친구 신청한 친구)
 GET /friend/me/mutualfriend => 서로 친구 조회
 ...
 GET /friend/me/recommendation => 친구 추천 (test 필요)
*/
...
router.get('/me/mutualfriend', async function(req, res) { // member_id의 서로 친구 조회
  console.log('GET /api/friend');
  var memberID = req.decoded._objID;
  
  var mutualFriendList = await friendQuery.getMutualFriendList(memberID);
  
  if (mutualFriendList)
    return res.json(util.successTrue(mutualFriendList));
  else
    return res.json(util.successFalse(mutualFriendList))
});
...
router.get('/me/recommendation', async function(req, res) { // member_id가 추가한 친구 조회
  ...
  .then( async (friends) => {
    ...
    if (mutualFriends.hasOwnProperty(memberID)) {
      delete mutualFriends[memberID];
    }
    
    var mutualFriendList = [];
    for (var key in mutualFriends) {
      // check if the property/key is defined in the object itself, not in parent
      if (mutualFriends.hasOwnProperty(key)) {           
          //console.log(key, mutualFriends[key]);
          mutualFriendList.push(mutualFriends[key]);
      }
    }
