...
/*
 ...
 DELETE /board/:id/post/:post_id => 상세 게시물 삭제

 GET /board/:id/post/:post_id/comment => 댓글 조회
 POST /board/:id/post/:post_id/comment => 댓글 작성
 DELETE /board/:id/post/:post_id/comment/:comment_id => 댓글 삭제
 ...
 GET /board/:id/schedule => 여행 일정 조회(7주일 이내 간격시 하나의 리스트로 묶어주어 반환)
*/
...
// 여행 일정 조회
router.get('/:id/schedule', async function(req, res) {
  console.log('GET /api/board/:id/schedule')
  //var memberID = req.decoded._id;
  var memberID = req.decoded._objID;
  var boardID = req.params.id;
  ...
  if (postList.length >= 1) {
    ...
    scheduleCollection['endTime'] = postList[0].postTime;
    scheduleCollection['startTime'] = postList[0].postTime;
    scheduleCollection['postList'] = [];
    scheduleCollection['postList'].push(postList[0])
    var currentTime = postList[0].postTime;

    for (var i = 1; i < postList.length; ++i) {
      var diffDay = util.daysBetween(postList[i].postTime, currentTime);
      console.log(diffDay);
      if (diffDay > 7) {
        scheduleList.push(scheduleCollection)
        var scheduleCollection = {};
        scheduleCollection['endTime'] = postList[i].postTime;
        scheduleCollection['startTime'] = postList[i].postTime;
        scheduleCollection['postList'] = [];
        scheduleCollection['postList'].push(postList[i])
        currentTime = postList[i].postTime;
      }
      else {
        scheduleCollection['startTime'] = postList[i].postTime;
        scheduleCollection['postList'].push(postList[i])
  ...
  res.json(util.successTrue(scheduleList));
});
...
// 상세 게시물 수정
router.put('/:id/post/:post_id', upload.array('photos[]'), async function(req, res) {
...
// 상세 게시물 삭제
router.delete('/:id/post/:post_id', async function(req, res) {
  console.log('DELETE /board/:id/post/:post_id');
  //var memberID = req.decoded._id;
  var memberID = req.decoded._objID;
  var boardID = req.params.id;
  var postID = req.params.post_id;
  ...
  if (!post) {
    ...
    console.log('member can not modify post..');
    return res.json(util.successFalse('member can not remove post!'));
  }
  //console.log(readPermission);
  ...
  .then( (post) => {
    ...
    post.delete(function (err, post) {
      if(err) {
        ...
        console.log('와웅');
        return res.send(util.successTrue('delete success!!')); // 수정된 게시물 반환
      } 
    });
...
// 댓글 수정
router.put('/:id/post/:post_id/comment/:comment_id', async function(req, res) {
  console.log('DELETE /board/:id/post/:post_id/comment/:comment_id');
  var memberID = req.decoded._objID;
  var boardID = req.params.id;
  var postID = req.params.post_id;
  ...
  if (comment.commentWriterID != memberID) {
    return res.json(util.successFalse("you can not remove comment!"));
  }
  else {
    var contents = req.body.contents;
    comment.commentContents = contents;
    comment.save(function(err, comment) {
      if (err) res.json(util.successFalse("fail remove comment!"));
      else res.json(util.successTrue("success remove comment!"));
    });
  }
});
...
// 댓글 삭제
router.delete('/:id/post/:post_id/comment/:comment_id', async function(req, res) {
  console.log('DELETE /board/:id/post/:post_id/comment/:comment_id');
  var memberID = req.decoded._objID;
  var boardID = req.params.id;
  var postID = req.params.post_id;
  ...
  if (comment.commentWriterID != memberID) {
    ...
    var post = await postQuery.getPost(postID);
    var index = post.commentList.indexOf(commentID);
    if (index > -1) 
    {
