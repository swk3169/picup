...
router.post('/', upload.single('memberProfile'), async function(req, res, next){
  ...
  if (!file) { // profile copy 부분 추가
    return res.json(util.successFalse('member register fail!!'));
  }
...
// 프로필 수정
router.put('/me', upload.single('memberProfile'), async function(req, res) {
  console.log('PUT /member/me');
  var memberID = req.decoded._id;
  
  var member = await memberQuery.findMemberBySNSID(memberID);
  var memberObjID = member._id;

  var boardID = member.privateBoard;
  ...
  //console.log(req.body.get('memberName'));
  console.log(email);
  console.log(birth);
  console.log(name);
  console.log(file);
  //return res.send('temprary success!');
  ...
  if (file && util.isImage(file.mimetype)) {
    console.log('profile image modify!')
    var dir = file.destination;
    var resourcedir = dir.substr(dir.indexOf('/') + 1);
    ...
    image.resize(50, 50).write(newfilepath); // resize image file and save
    fs.unlinkSync(filepath);  // remove original file
    modifyInfo.memberProfile = resourcefilepath
    boardInfo.boardProfile = resourcefilepath
  }

  const transaction = new Transaction();
  try {

    transaction.update('board', boardID, boardInfo);
    transaction.update('member', memberObjID, modifyInfo);

    const final = await transaction.run()

    return res.json(util.successTrue('success update!'));
  ...
  /*
  Member.findOneAndUpdate({memberID: memberID}, modifyInfo, {new: true})
  .then( (member) => {
    console.log(member);
    return res.send(util.successTrue(member)); // 수정된 게시물 반환
  })
  .catch( (err) => {
    console.log(err);
    return res.send(util.successFalse('can not found member!'));
  });
  */
});
